CREATE TABLE [dbo].[DavisVantageProConsoleRecords_OneMinuteArchive] (
    ...
    CONSTRAINT [PK_DavisVantageProConsoleRecords_OneMinuteArchive] PRIMARY KEY CLUSTERED ([id] ASC)
);


GO

CREATE UNIQUE NONCLUSTERED INDEX [IX_DavisVantageProConsoleRecords_OneMinuteArchive_RecordDateTime]
    ON [dbo].[DavisVantageProConsoleRecords_OneMinuteArchive]([recordDateTime] ASC);


GO

CREATE NONCLUSTERED INDEX [IX_DavisVantageProConsoleRecords_OneMinuteArchive_RecordDateTime_Cover]
    ON [dbo].[DavisVantageProConsoleRecords_OneMinuteArchive]([recordDateTime] ASC)
    INCLUDE([barometer], [insideTemperature], [insideHumidity], [outsideTemperature], [outsideHumidity], [windSpeed], [windDirection], [rainRate], [uvIndex], [solarRadiation], [stormRain], [stormStartDate], [dailyRainAmount], [monthlyRainAmount], [yearlyRainAmount], [consoleBatteryVoltage], [sunriseTime], [sunsetTime], [dailyETAmount], [monthlyETAmount], [yearlyETAmount], [outsideHeatIndex], [outsideWindChill], [outsideDewpoint]);
