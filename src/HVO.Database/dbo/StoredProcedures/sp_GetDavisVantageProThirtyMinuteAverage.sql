...
CREATE PROCEDURE [dbo].[sp_GetDavisVantageProThirtyMinuteAverage] 
...
BEGIN
  ...
  SELECT 
    --MIN(DATEADD(MINUTE, FLOOR(recordDateTimeMinute / 30) * 30, DATEADD(MINUTE, DATEPART(MINUTE, recordDateTime) * -1, DATEADD(SECOND, DATEPART(SECOND, recordDateTime) * -1, DATEADD(nanosecond, DATEPART(nanosecond, recordDateTime) * -1, recordDateTime))))) as recordDateTime,
    MIN(DATEADD(MINUTE, FLOOR(DATEPART(minute, recordDateTime) / 30) * 30, DATEADD(MINUTE, DATEPART(MINUTE, recordDateTime) * -1, DATEADD(SECOND, DATEPART(SECOND, recordDateTime) * -1, DATEADD(nanosecond, DATEPART(nanosecond, recordDateTime) * -1, recordDateTime))))) as recordDateTime,
  ...
  GROUP BY
    --recordDateTimeYear, recordDateTimeMonth, recordDateTimeDay, recordDateTimeHour, (FLOOR(recordDateTimeMinute / 30) * 30)
    DATEPART(year, recordDateTime), DATEPART(month, recordDateTime), DATEPART(day, recordDateTime), DATEPART(hour, recordDateTime), (FLOOR(DATEPART(minute, recordDateTime) / 30) * 30)
  ORDER BY
    recordDateTime DESC
