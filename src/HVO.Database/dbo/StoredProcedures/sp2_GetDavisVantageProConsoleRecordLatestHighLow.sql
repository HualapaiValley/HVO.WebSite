...
CREATE PROCEDURE [dbo].[sp2_GetDavisVantageProConsoleRecordLatestHighLow] 
...
BEGIN
  ...
  WITH LatestRecord AS (
      SELECT 
	    RecordDateTime as LatestRecordDateTime
		,Barometer
		,InsideTemperature
		,InsideHumidity
		,OutsideTemperature
		...
		,OutsideWindChill
		,OutsideDewpoint
		,DailyRainAmount as RainDailyAmount
		,MonthlyRainAmount as RainMonthlyAmount
		,RainRate
		,StormRain as RainStormRain
		,StormStartDate as RainStormStartDate
		,YearlyRainAmount as RainYearlyAmount
	  FROM
        [DavisVantageProConsoleRecords_NEW]
	  WHERE
	    recordDateTime = (SELECT MAX(recordDateTime) FROM [DavisVantageProConsoleRecords_NEW] WHERE recordDateTime BETWEEN @iStartRecordDateTime AND @iEndRecordDateTime)
  ),
