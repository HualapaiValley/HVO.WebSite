...
CREATE PROCEDURE [dbo].[sp_GetDavisVantageProFifteenMinuteAverage] 
...
BEGIN
  ...
  SELECT                        
--    MIN(DATEADD(MINUTE, FLOOR(recordDateTimeMinute / 15) * 15, DATEADD(MINUTE, DATEPART(MINUTE, recordDateTime) * -1, DATEADD(SECOND, DATEPART(SECOND, recordDateTime) * -1, DATEADD(nanosecond, DATEPART(nanosecond, recordDateTime) * -1, recordDateTime))))) as recordDateTime,
  ...
  GROUP BY
--    recordDateTimeYear, recordDateTimeMonth, recordDateTimeDay, recordDateTimeHour, (FLOOR(recordDateTimeMinute / 15) * 15)
