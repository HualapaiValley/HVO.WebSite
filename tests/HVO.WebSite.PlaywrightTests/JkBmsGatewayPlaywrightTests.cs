...
[TestClass]
public sealed class JkBmsGatewayPlaywrightTests
{
    [TestMethod]
    [TestCategory("Live")]
    public async Task JkBmsGateway_ShouldRenderLayoutShellAndKeepCircuitAlive()
    {
        ...
        await Assertions.Expect(page.Locator(".shell-brand-subtitle")).ToContainTextAsync("JK BMS DASHBOARD");
        await Assertions.Expect(page.GetByRole(AriaRole.Link, new() { Name = "Overview" })).ToBeVisibleAsync();
        await Assertions.Expect(page.GetByRole(AriaRole.Link, new() { Name = "Banks" })).ToBeVisibleAsync();
    ...
    public async Task JkBmsGateway_ShouldNavigateOverviewBanksAndKeepCircuitAlive()
    {
        ...
        await page.GetByRole(AriaRole.Link, new() { Name = "Banks" }).ClickAsync();
        await Assertions.Expect(page).ToHaveURLAsync(new Regex("/devices/?$"));
        await Assertions.Expect(page.GetByRole(AriaRole.Heading, new() { Name = "Configured JK BMS banks" })).ToBeVisibleAsync();
        await Assertions.Expect(page.GetByRole(AriaRole.Heading, new() { Name = "Poller device inventory" })).ToBeVisibleAsync();
        await AssertNoBlazorErrorAsync(page);
    ...
    public async Task JkBmsGateway_ShouldRenderThemedCardsAndControls()
    {
        ...
        await AssertThemedSurfaceAsync(page.Locator(".jk-card").First, "Overview card");
        await AssertThemedSurfaceAsync(page.Locator(".jk-gauge").First, "Overview gauge");
        await AssertThemedSurfaceAsync(page.Locator(".jk-stat-chip").First, "Overview stat chip");
        await AssertNoLegacyClassesAsync(page, "Overview");
        await AssertNoBlazorErrorAsync(page);

        await page.GotoAsync(BuildUrl("/devices"));
        await AssertThemedSurfaceAsync(page.Locator(".jk-table-shell").First, "Banks table shell");
        await AssertNoLegacyClassesAsync(page, "Banks");
        await AssertNoBlazorErrorAsync(page);
    }
    ...
    public async Task JkBmsGateway_ShouldRenderDeviceDetailCellChart_WhenConfiguredDeviceExists()
    {
        ...
        if (await deviceLinks.CountAsync() == 0)
        {
            Assert.Inconclusive("The JkBms gateway has no configured device links to validate the device detail chart.");
        }

        await deviceLinks.First.ClickAsync();
        await AssertNoBlazorErrorAsync(page);

        if (await page.GetByText("Waiting for first reading").CountAsync() > 0)
        {
            Assert.Inconclusive("The selected JkBms device has no latest reading, so the cell chart is not expected yet.");
        }

        await Assertions.Expect(page.Locator("#jk-cell-chart")).ToBeVisibleAsync();
        await page.WaitForFunctionAsync("() => Boolean(window.hvoChart?._instances?.['jk-cell-chart'])");
        var chartWidth = await page.Locator("#jk-cell-chart").EvaluateAsync<int>("canvas => canvas.clientWidth");
    ...
    private static async Task<PlaywrightBrowserSession> CreateBrowserSessionAsync()
    {
        ...
        {
            Assert.Inconclusive("Set HVO_JKBMS_BASE_URL to run the JK BMS gateway UI Playwright tests.");
        }
